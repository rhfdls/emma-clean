...
function Test-PostgresRunning {
    if (Test-CommandExists "psql") {
        try {
            # Using proper PowerShell SQL command formatting with double quotes
            psql -U postgres -d emma_dev -c "\q" 2>$null
            return $true
        } catch {
            return $false
        }
...
function Verify-RequiredEnvironmentVariables {
    ...
    # if ([string]::IsNullOrEmpty($env:VARIABLE_NAME)) {
    #     $missingVars += "VARIABLE_NAME"
    # }
...
function Apply-EfMigrations {
    ...
    if (Test-PostgresRunning) {
        ...
        try {
            # Note: based on the memory, direct dotnet commands are now allowed
