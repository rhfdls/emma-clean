    ...
    public class TaskService : ITaskService
    {
        ...
        public async Task<IEnumerable<TaskItem>> GetTasksAsync(TaskFilter filter, Guid requestingUserId)
        {
            ...
            if (!filter.IncludeCompleted)
                query = query.Where(t => t.Status != Emma.Models.Enums.TaskStatus.Completed && t.Status != Emma.Models.Enums.TaskStatus.Cancelled);
        ...
        public async Task<TaskItem> CreateTaskAsync(TaskItem task, Guid createdByUserId)
        {
            ...
            if (task.Status == default)
                task.Status = Emma.Models.Enums.TaskStatus.Pending;
        ...
        public async Task<TaskItem> UpdateEmma.Models.Enums.TaskStatusAsync(string taskId, Emma.Models.Enums.TaskStatus newStatus, Guid updatedByUserId, string? notes = null)
        {
            var task = await _context.TaskItems.FindAsync(taskId);
            if (task == null)
                throw new NotFoundException($"Task with ID {taskId} not found");
        ...
        {
            return await UpdateEmma.Models.Enums.TaskStatusAsync(taskId, Emma.Models.Enums.TaskStatus.Completed, completedByUserId, notes);
        }
            ...
            bool requiresApproval = false)
        {
            ...
            var task = new TaskItem
            {
                ...
                DueDate = dueDate,
                ContactId = contactId,
                Status = requiresApproval ? Emma.Models.Enums.TaskStatus.PendingApproval : Emma.Models.Enums.TaskStatus.Pending,
        ...
        public async Task<bool> ValidateNbaTaskAsync(string taskId, bool isValid, string? reason, Guid validatedByUserId)
        {
            ...
            if (!isValid)
            {
                task.Status = Emma.Models.Enums.TaskStatus.Rejected;
            }
            // If task is valid and was pending approval, move to pending
            else if (task.Status == Emma.Models.Enums.TaskStatus.PendingApproval)
            {
                task.Status = Emma.Models.Enums.TaskStatus.Pending;
            }
        ...
        public async Task<TaskStatistics> GetTaskStatisticsAsync(TaskStatisticsFilter filter, Guid requestingUserId)
        {
            ...
            var stats = new TaskStatistics
            {
                ...
                CompletedTasks = tasks.Count(t => t.Status == Emma.Models.Enums.TaskStatus.Completed),
                OverdueTasks = tasks.Count(t => t.DueDate < DateTime.UtcNow && 
                                              t.Status != Emma.Models.Enums.TaskStatus.Completed && 
                                              t.Status != Emma.Models.Enums.TaskStatus.Cancelled),
                DueSoonTasks = tasks.Count(t => t.DueDate > DateTime.UtcNow && 
                                              t.DueDate <= DateTime.UtcNow.AddDays(7) &&
                                              t.Status != Emma.Models.Enums.TaskStatus.Completed && 
                                              t.Status != Emma.Models.Enums.TaskStatus.Cancelled),
                NotStartedTasks = tasks.Count(t => t.Status == Emma.Models.Enums.TaskStatus.NotStarted),
                InProgressTasks = tasks.Count(t => t.Status == Emma.Models.Enums.TaskStatus.InProgress),
            ...
            var completedTasks = tasks.Where(t => t.Status == Emma.Models.Enums.TaskStatus.Completed).ToList();
